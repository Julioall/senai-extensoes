(() => {
  ...
  function questionTitle() {
    ...
    return selectors.map(selector => [...document.querySelectorAll(selector)].find(isVisible)).find(Boolean) || null;
  }
  ...
  function answerCards() {
    ...
    const inputs = [...document.querySelectorAll('[data-functional-selector="question-answer__input"], [contenteditable="true"][aria-label*="answer" i], [contenteditable="true"][aria-label*="resposta" i], textarea[placeholder*="answer" i], input[placeholder*="answer" i]')]
      .filter(element => isVisible(element) && element !== questionTitle()).slice(0, 4);
    return inputs.map(input => input.closest('[data-functional-selector*="question-answer"],article,li,div') || input);
  }

  function answerInput(card, index) {
    const stable = card?.querySelector?.('[data-functional-selector="question-answer__input"]');
    if (stable) return stable;
    const direct = card?.matches?.('input,textarea,[contenteditable="true"]') ? card : null;
    if (direct) return direct;
    const candidate = card?.querySelector?.('input[type="text"],textarea,[contenteditable="true"]');
    if (candidate) return candidate;
    return [...document.querySelectorAll('[data-functional-selector="question-answer__input"]')].filter(isVisible)[index] || null;
  }

  function correctToggles() {
    const selectors = [
      '[data-functional-selector="question-answer__toggle-button"]',
      'button[data-functional-selector*="answer"][aria-checked]',
      'button[data-functional-selector*="answer"][aria-pressed]',
  ...
  function currentQuestionIsBlank() {
    ...
    return editorText(title) === '' && cards.length >= 4 && cards.every((card, index) => editorText(answerInput(card, index)) === '');
  }

  function diagnostics() {
    const missing = [];
    if (!questionTitle()) missing.push('campo da pergunta');
    if (answerCards().length < 4) missing.push('quatro alternativas');
    if (correctToggles().length < 4) missing.push('seleção da alternativa correta');
  ...
  function insertText(element, value) {
    ...
    let inserted = false;
    try { inserted = document.execCommand('insertText', false, String(value)); } catch { inserted = false; }
    if (!inserted) {
  ...
  function toggleSelected(toggle) {
    return toggle.getAttribute('aria-checked') === 'true' || toggle.getAttribute('aria-pressed') === 'true' || toggle.dataset.selected === 'true' || toggle.classList.contains('selected') || toggle.classList.contains('is-selected');
  }

  async function setCorrectAnswer(correctIndex) {
    const toggles = correctToggles();
    if (toggles.length < 4) throw new Error('A seleção da resposta correta não foi localizada.');
    for (let index = 0; index < 4; index += 1) {
      if (toggleSelected(toggles[index]) !== (index === correctIndex)) {
        toggles[index].scrollIntoView({ block: 'center', inline: 'nearest' });
        toggles[index].click();
        await sleep(220);
      }
    }
  ...
  async function fillCurrentQuestion(item) {
    ...
    for (let index = 0; index < 4; index += 1) {
      await replaceText(() => answerInput(answerCards()[index], index), item.answers[index], `a alternativa ${index + 1}`);
    }
  ...
  function controlledPopup(button) {
    ...
    return [...document.querySelectorAll('[role="dialog"],[role="menu"],[data-radix-popper-content-wrapper]')].filter(isVisible).at(-1) || null;
  }

  function quizOption(root) {
    if (!root) return null;
    return [...root.querySelectorAll('button,[role="button"],[data-functional-selector]')].filter(isVisible).find(element => {
      const text = normalized(element.innerText || element.textContent);
  ...
  async function createNewQuestion() {
    ...
    await waitFor(() => sidebarBlocks().length > before || questionTitle() !== previous, 'a nova pergunta', 12000);
  ...
  function createProgress(total) {
    ...
    panel.innerHTML = `<header><span class="sx-kahoot-progress__brand" aria-hidden="true">S</span><strong data-title>Importando questões</strong><button type="button" data-minimize aria-label="Minimizar">−</button></header><div class="sx-kahoot-progress__body"><div class="sx-kahoot-progress__line"><span data-count>0 de ${total} questões</span><small data-percent>0%</small></div><div class="sx-kahoot-progress__track"><span data-bar></span></div></div><footer><span data-time>◷ 00:00</span><button type="button" data-hide>Ocultar</button><button type="button" data-cancel aria-label="Cancelar importação">×</button></footer>`;
  ...
  async function runImport(questions) {
    ...
    try {
      ...
      updateProgress(0, questions.length, aborted ? 'warning' : 'error', aborted ? 'Importação cancelada' : 'Falha na importação');
  ...
  function stepper(active) {
    return `<nav class="sx-kahoot-stepper" aria-label="Etapas da importação">${['Adicionar', 'Validar', 'Executar'].map((label, index) => {
      const step = index + 1;
      const complete = step < active;
      return `<div class="${step === active ? 'is-active' : ''} ${complete ? 'is-complete' : ''}"><span>${complete ? '✓' : step}</span><b>${step}. ${label}</b></div>`;
    }).join('')}</nav>`;
  }
  ...
  function openModal() {
    ...
    overlay.innerHTML = `<section class="sx-kahoot-modal" role="dialog" aria-modal="true" aria-labelledby="sx-kahoot-title"><header class="sx-kahoot-modal__header"><span class="sx-brand-mark" aria-hidden="true">S</span><div><h2 id="sx-kahoot-title">Importar questões</h2><p>Fluxo guiado para adicionar perguntas ao quiz</p></div><button type="button" class="sx-icon-button" data-close aria-label="Fechar"><svg class="sx-icon" viewBox="0 0 24 24"><path d="M18 6 6 18M6 6l12 12"/></svg></button></header><div class="sx-kahoot-modal__content"></div></section>`;
  ...
  function renderAdd(content) {
    content.innerHTML = `${stepper(1)}<section class="sx-kahoot-stage"><h3>Adicionar arquivo</h3><label class="sx-kahoot-dropzone"><input type="file" accept=".csv,.tsv,.txt,text/csv"><svg class="sx-kahoot-file-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M14 2H6a2 2 0 0 0-2 2v16h16V8Z"/><path d="M14 2v6h6M8 13h8M8 17h6"/></svg><strong>${state.file ? escapeHtml(state.file.name) : 'Arraste o arquivo aqui'}</strong><span>${state.file ? 'Clique para trocar o arquivo' : 'ou selecione no computador'}</span><span class="sx-button sx-button--secondary">Selecionar arquivo</span></label><div class="sx-kahoot-info"><b>Formato aceito: CSV (UTF-8).</b><span>Colunas esperadas: pergunta, opcao1, opcao2, opcao3, opcao4, correta.</span></div><div class="sx-kahoot-options"><h4>Opções de importação</h4><button type="button" data-mode="replace" class="${state.mode === 'replace' ? 'is-selected' : ''}"><span>Substituir pergunta atual</span><i></i></button><button type="button" data-mode="append" class="${state.mode === 'append' ? 'is-selected' : ''}"><span>Adicionar ao final do quiz</span><i></i></button></div></section><footer class="sx-kahoot-actions"><button type="button" class="sx-button sx-button--secondary" data-cancel>Cancelar</button><button type="button" class="sx-button" data-validate ${state.file ? '' : 'disabled'}>Validar arquivo</button></footer>`;
    const input = content.querySelector('input[type="file"]');
    input.addEventListener('change', () => { state.file = input.files?.[0] || null; state.parsed = null; renderModal(); });
    content.querySelectorAll('[data-mode]').forEach(button => button.addEventListener('click', async () => {
      state.mode = button.dataset.mode;
      state.settings = await updateSettings({ kahoot: { replaceCurrent: state.mode === 'replace' } });
      renderModal();
    }));
  ...
  function renderValidation(content) {
    ...
    content.innerHTML = `${stepper(2)}<section class="sx-kahoot-stage"><h3>Arquivo carregado</h3><div class="sx-kahoot-file-card"><svg class="sx-icon" viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16h16V8Z"/><path d="M14 2v6h6M8 13h8M8 17h6"/></svg><span><b>${escapeHtml(state.file?.name || 'Arquivo CSV')}</b><small>Arquivo lido com sucesso</small></span></div><h3>Resumo da validação</h3><div class="sx-kahoot-metrics"><span><b>${questions.length}</b>perguntas</span><span><b>${questions.length * 4}</b>alternativas</span><span><b>${questions.length}</b>respostas corretas</span><span class="${errors.length ? 'is-error' : ''}"><b>${errors.length}</b>erros críticos</span></div><h3>Pré-visualização</h3><div class="sx-kahoot-preview">${questions.slice(0, 3).map((question, index) => `<div><span>${index + 1}</span><p>${escapeHtml(question.question)}</p><b>OK</b></div>`).join('')}</div><div class="sx-kahoot-info"><b>${errors.length ? `${errors.length} problema(s) precisam ser corrigidos.` : 'Todas as colunas obrigatórias foram encontradas.'}</b><span>As perguntas serão adicionadas ao quiz após a execução.</span></div></section><footer class="sx-kahoot-actions"><button type="button" class="sx-button sx-button--secondary" data-back>Voltar</button><button type="button" class="sx-button" data-run ${canRun ? '' : 'disabled'}>Executar importação</button></footer>`;
    content.querySelector('[data-back]').addEventListener('click', () => { state.step = 1; renderModal(); });
    content.querySelector('[data-run]').addEventListener('click', () => { const items = [...questions]; closeModal(); runImport(items); });
  }

  function importButtonHost() {
    const add = addQuestionButton();
    if (!add) return null;
    const create = [...document.querySelectorAll('button,[role="button"]')].filter(isVisible).find(element => ['create', 'criar'].includes(normalized(element.innerText || element.textContent)));
    if (create && create.parentElement === add.parentElement) return { parent: create.parentElement, after: create };
  ...
  function ensureImportButton() {
    ...
    if (!button) {
      ...
      button.innerHTML = `<svg class="sx-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3v12m-5-5 5 5 5-5"/><path d="M5 19h14"/></svg><span>Importar!</span>`;
      button.addEventListener('click', openModal);
    }
    const host = importButtonHost();
    if (host && button.parentElement !== host.parent) host.after.insertAdjacentElement('afterend', button);
    else if (!host && !button.isConnected) { button.classList.add('is-floating'); document.body.appendChild(button); }
    else if (host) button.classList.remove('is-floating');
  }
